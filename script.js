...
function checkPassword() {
  ...
  if (
    ...
    passwordError.innerHTML =
      '<i class="fa-solid fa-circle-info"></i> ' + 'Enter A Strong Password';
    passwordError.addEventListener('click', function () {
      if (!alertShown) {
        alert(
          ' requires a password to have at least 8 characters, including at least one uppercase letter, one lowercase letter, one digit, and one special character'
        );
        alertShown = true;
      }
    });
...
function validate() {
  ...
  text.innerHTML = '<i class="fas fa-check-double"></i>';
  successScreen.appendChild(text);
